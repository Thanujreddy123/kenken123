...
function displayRules() {
    // Open rules.html in a new window
    const rulesWindow = window.open('rules.html', '_blank');
    if (rulesWindow) {
        rulesWindow.focus();
    } else {
        alert('Please allow pop-ups to view the rules.');
    }
}

// Attach this function to the rules button click event
...
function makeBoard() {
    ...
    dimension = parseInt(document.getElementById('dim-input').value);
    if (isNaN(dimension) || dimension < 2 || dimension > 9) {
        document.getElementById('input-error').classList.remove('invis');
        return;
    }
    ...
    square_size = Math.floor((100 / dimension) * .55);
...
function solve(hint) {
    ...
    if (boards.length == 0) {
        ...
        document.getElementById('result').innerHTML = 'There is something wrong in the input can you please check the input';
    ...
    if (boards.length > 1) {
        ...
        document.getElementById('result').innerHTML = 'Successful You got the Ouptut';
    };
    let board = boards[0];
...
function getBoard() {
    ...
    for (let box of boxes) {
        let possible_boxes;
        if (box.operation == ' ') {
            possible_boxes = noOpTo(box.number);
        }
